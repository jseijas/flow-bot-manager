...
var BotManager = function () {
  ...
  _createClass(BotManager, [{
    ...
    value: function createConnector() {
      ...
      this.connector = new _flowConnector.FlowMultiConnector();
    ...
    value: function createRenderFactory() {
      this.renderFactory = new _flowRenderFactory.FlowRenderFactory({ template: this.template, builder: _botbuilder2.default, parent: this });
    }
  }, {
    key: 'getAbsolutePath',
    ...
    value: function addCard(item) {
      ...
      if (item.type === 'prompt' && item.prompt === 'choice' && _lodash2.default.isString(item.options)) {
